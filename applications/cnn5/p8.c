 ...
 * @date November 2024
 *
 * @brief Layer 4 (AlexNet Conv4) accumulator: receives partial results from
 *        p7_x workers, accumulates, applies bias and relu. No max pooling
 *        after Conv4 in AlexNet. Then dispatches each channel to p9_x workers
 *        round-robin for Conv5.
 */
...
int main()
{
	...
	unsigned long long *time_rec  = malloc(OUT_DEPTH_4*sizeof(unsigned long long));
    unsigned long long *time_recd = malloc(OUT_DEPTH_4*sizeof(unsigned long long));
    unsigned long long *time_end  = malloc(OUT_DEPTH_4*sizeof(unsigned long long));
	unsigned long long *time_sent = malloc(OUT_DEPTH_4*sizeof(unsigned long long));

	int *msg_0 = malloc(P7_MSG_SIZE*sizeof(int));
	int *msg_1 = malloc(P7_MSG_SIZE*sizeof(int));
	int *msg_2 = malloc(P7_MSG_SIZE*sizeof(int));
	int *msg_3 = malloc(P7_MSG_SIZE*sizeof(int));
	int *msg_4 = malloc(P7_MSG_SIZE*sizeof(int));
	int *msg_5 = malloc(P7_MSG_SIZE*sizeof(int));

    printf("Starting P8 - Ticks = %s\n\n", memphis_u64_str(memphis_get_tick()));

	int *out_4 = malloc(OUT_CONV_WIDTH_4 * OUT_CONV_HEIGHT_4 * OUT_DEPTH_4*sizeof(int));
	/* Inicializa out_4 */
    for (int i = 0; i < OUT_CONV_WIDTH_4 * OUT_CONV_HEIGHT_4 * OUT_DEPTH_4; i++)
    {
        out_4[i] = 0;
    }
	...
	for(int o_channel = 0; o_channel < OUT_DEPTH_4; o_channel++){
		...
		int received_2 = memphis_receive(msg_2, P7_MSG_SIZE*sizeof(int), p7_2);
		int received_3 = memphis_receive(msg_3, P7_MSG_SIZE*sizeof(int), p7_3);
		int received_4 = memphis_receive(msg_4, P7_MSG_SIZE*sizeof(int), p7_4);
        ...
        conv_sum_bias(OUT_CONV_HEIGHT_4, OUT_CONV_WIDTH_4, out_slice, bias_4, o_channel);

		/* RELU */
		relu(out_slice, OUT_CONV_HEIGHT_4, OUT_CONV_WIDTH_4, 1);

		/* No max pooling after Conv4 in AlexNet */

		/* Dispatch to p9_x workers round-robin by output channel */
		switch (o_channel % P9_AMONT_OF_PES) {
			case 1:
				memphis_send(out_slice, P8_MSG_SIZE*sizeof(int), p9_1);
				break;
			case 2:
				memphis_send(out_slice, P8_MSG_SIZE*sizeof(int), p9_2);
				break;
			case 3:
				memphis_send(out_slice, P8_MSG_SIZE*sizeof(int), p9_3);
				break;
			default:
				memphis_send(out_slice, P8_MSG_SIZE*sizeof(int), p9_0);
		}
